...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfileData() async {
    ...
    setState(() {
      _nameController.text = _profileBox.get('name', defaultValue: 'سارة أحمد');
      _emailController.text = _profileBox.get('email', defaultValue: 'sara@example.com');
      _phoneController.text = _profileBox.get('phone', defaultValue: '+966 5XXXXXXXX');
      _notificationsEnabled = _profileBox.get('notificationsEnabled', defaultValue: true);
      _aiRecommendations = _profileBox.get('aiRecommendations', defaultValue: true);
      _darkMode = _profileBox.get('darkMode', defaultValue: false);
      _isLoading = false;
    });
    
    // تحميل الصورة إذا وجدت
    final imagePath = _profileBox.get('profileImage');
    if (imagePath != null) {
  ...
  Future<void> _saveProfileData() async {
    ...
    await _profileBox.put('email', _emailController.text);
    await _profileBox.put('phone', _phoneController.text);
    await _profileBox.put('notificationsEnabled', _notificationsEnabled);
    await _profileBox.put('aiRecommendations', _aiRecommendations);
    await _profileBox.put('darkMode', _darkMode);
    
    if (_profileImage != null) {
      await _profileBox.put('profileImage', _profileImage!.path);
    }
  }
  ...
  Future<void> _changePassword() async {
    ...
    await _profileBox.put('password', _newPasswordController.text);
    Get.back();
    ...
    try {
      ...
      final exportData = {
        ...
        'stats': {
          ...
          'completionRate': tasks.isEmpty ? 0 : (tasks.where((t) => t.status == TaskStatus.completed).length / tasks.length * 100).round(),
        },
      ...
      final file = File('${directory.path}/secondmind_backup_${DateTime.now().millisecondsSinceEpoch}.json');
      await file.writeAsString(jsonString);

      await Share.shareXFiles([XFile(file.path)], text: '📊 تصدير بيانات SecondMind');
  ...
  Future<void> _deleteAccount() async {
    final confirmed = await Get.dialog<bool>(
      AlertDialog(
        ...
        title: Text('حذف الحساب', style: AppTheme.headlineMd.copyWith(color: AppTheme.error)),
        content: const Text('هل أنت متأكد من حذف حسابك؟ سيتم حذف جميع بياناتك نهائياً.'),
        actions: [
...
https://secondmind.app/download
  ...
  void _toggleDarkMode(bool value) async {
    ...
    _showSuccessSnackbar(value ? 'تم تفعيل الوضع المظلم' : 'تم تفعيل الوضع الفاتح');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final streakDays = taskController.streakDays;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            _buildStatsCard(completedTasks, totalTasks, completionRate, streakDays),
            const SizedBox(height: 20),
      ...
      actions: [
        if (!_isEditing)
          TextButton.icon(
            ...
            label: Text('تعديل', style: AppTheme.labelMd.copyWith(color: AppTheme.primary)),
          )
        else
          Row(
            children: [
              TextButton.icon(
                ...
                label: Text('إلغاء', style: AppTheme.labelMd.copyWith(color: AppTheme.error)),
              ),
              TextButton.icon(
                onPressed: _saveProfile,
                icon: Icon(Icons.check, size: 18, color: AppTheme.primary),
                label: Text('حفظ', style: AppTheme.labelMd.copyWith(color: AppTheme.primary)),
              ),
            ],
          ),
...
Widget _buildInfoCard() {
        ...
        AnimatedContainer(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              filled: true,
              fillColor: enabled ? AppTheme.surfaceContainerLow : AppTheme.surfaceContainerLowest,
  ...
  // STATS CARD
  //============================================================================
Widget _buildStatsCard(int completed, int total, int rate, int streak) {
  return Card(
    color: AppTheme.surfaceContainerLowest,
    elevation: 0,
    shape: RoundedRectangleBorder(
      ...
      child: Row(
        children: [
          Expanded(
            child: Column(
              children: [
                Icon(Icons.check_circle_outline, size: 28, color: AppTheme.statusCompleted),
                const SizedBox(height: 8),
                Text('$completed', style: AppTheme.headlineLg.copyWith(fontSize: 22, color: AppTheme.statusCompleted)),
          ...
          Expanded(
            child: Column(
              children: [
                ...
                const SizedBox(height: 8),
                Text('$rate%', style: AppTheme.headlineLg.copyWith(fontSize: 22, color: AppTheme.primary)),
                const SizedBox(height: 4),
          ...
          Expanded(
            child: Column(
              children: [
                Icon(Icons.local_fire_department, size: 28, color: AppTheme.statusPending),
                const SizedBox(height: 8),
                Text('$streak', style: AppTheme.headlineLg.copyWith(fontSize: 22, color: AppTheme.statusPending)),
                const SizedBox(height: 4),
                Text('أيام متتالية', style: AppTheme.labelSm),
              ],
            ),
          ),
        ],
    ...
    child: Column(
      ...
      children: [
        Padding(
          ...
          child: Text('التفضيلات', style: AppTheme.labelMd.copyWith(color: AppTheme.outline, letterSpacing: 0.5)),
        ),
        _buildSwitchTile('إشعارات المهام', 'تنبيهات للمهام القادمة', _notificationsEnabled, (v) async {
          setState(() => _notificationsEnabled = v);
          await _saveProfileData();
        }),
        const Divider(height: 1, indent: 20, endIndent: 20),
        _buildSwitchTile('توصيات الذكاء الاصطناعي', 'اقتراحات ذكية لتحسين إنتاجيتك', _aiRecommendations, (v) async {
          setState(() => _aiRecommendations = v);
          await _saveProfileData();
        }),
        const Divider(height: 1, indent: 20, endIndent: 20),
        _buildSwitchTile('الوضع المظلم', 'واجهة داكنة مريحة للعين', _darkMode, _toggleDarkMode),
      ],
    ),
  );
}
  ...
  Widget _buildSwitchTile(String title, String subtitle, bool value, Function(bool) onChanged) {
    return SwitchListTile(
      ...
      secondary: Container(
        ...
        child: Icon(value ? Icons.notifications_active : Icons.notifications_off, size: 20, color: value ? AppTheme.primary : AppTheme.outline),
      ),
    );
  }
  ...
  return Card(
    ...
    child: Column(
      ...
      children: [
        Padding(
          padding: const EdgeInsets.fromLTRB(20, 16, 20, 8),
          child: Text('الأمان', style: AppTheme.labelMd.copyWith(color: AppTheme.outline, letterSpacing: 0.5)),
        ),
        Material(
  ...
  return Card(
    ...
    child: Column(
      ...
      children: [
        Padding(
          padding: const EdgeInsets.fromLTRB(20, 16, 20, 8),
          child: Text('التطبيق', style: AppTheme.labelMd.copyWith(color: AppTheme.outline, letterSpacing: 0.5)),
        ),
        Material(
          color: Colors.transparent,
          child: ListTile(
        ...
        ListTile(
          ...
          trailing: Text('v1.0.0', style: AppTheme.labelSm.copyWith(color: AppTheme.outline)),
        ),
      ],
    ),
  );
  ...
  return Card(
    ...
    child: Material(
      ...
      child: ListTile(
        ...
        title: Text('تسجيل الخروج', style: AppTheme.bodyLg.copyWith(color: AppTheme.error, fontWeight: FontWeight.w600)),
