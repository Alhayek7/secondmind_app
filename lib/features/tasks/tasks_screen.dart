...
Widget build(BuildContext context) {
  ...
  return Obx(() {
    ...
    final completedTasks = controller.tasks.where((t) => t.status == TaskStatus.completed).length;
    ...
    return RefreshIndicator(
      ...
      child: CustomScrollView(
        ...
        slivers: [
          SliverToBoxAdapter(child: _buildWelcomeSection(controller, totalTasks, completedTasks, pendingTasks, completionRate)),
          const SliverToBoxAdapter(child: SizedBox(height: 16)),
          SliverToBoxAdapter(child: _buildStatsRow(controller, totalTasks, completedTasks)),
          const SliverToBoxAdapter(child: SizedBox(height: 16)),
                      ...
                      child: TaskCard(
                        ...
                        onStatusChanged: () async {
                          ...
                          if (oldStatus == TaskStatus.missed) {
                            ...
                            if (newDueDate == null) return; // المستخدم ألغى
                          }
    ...
    AlertDialog(
      ...
      actions: [
        ...
        TextButton(
          ...
          child: Text('غداً', style: AppTheme.labelMd.copyWith(color: AppTheme.primary)),
        ),
      ],
    ),
  );
...
void _handleStatusChange(TaskStatus oldStatus, TaskModel task, TaskController controller) {
  if (oldStatus == TaskStatus.inProgress && task.status == TaskStatus.completed) {
    ...
    Get.snackbar('🎉 إنجاز!', 'تم إكمال مهمة: ${task.title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.statusCompleted,
      colorText: Colors.white,
      duration: const Duration(seconds: 3),
    );
  } else if (oldStatus == TaskStatus.new_ && task.status == TaskStatus.inProgress) {
    SoundService.playFocusStartSound();
    Get.snackbar('🚀 بدء العمل!', 'بدأت العمل على مهمة: ${task.title}',
      snackPosition: SnackPosition.BOTTOM,
    ...
    Get.snackbar('🔄 إعادة فتح', 'تم إعادة فتح مهمة: ${task.title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.statusPending,
      colorText: Colors.white,
      duration: const Duration(seconds: 2),
    );
  } else if (oldStatus == TaskStatus.missed && task.status == TaskStatus.inProgress) {
    SoundService.playFocusStartSound();
    Get.snackbar('🔄 إعادة فتح', 'تم إعادة فتح المهمة الفائتة: ${task.title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.primary,
      colorText: Colors.white,
    );
  ...
  Widget _buildWelcomeSection(TaskController controller, int totalTasks, int completedTasks, int pendingTasks, int completionRate) {
        ...
        children: [
          ...
          Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text('التقدم اليومي', style: AppTheme.labelSm.copyWith(color: AppTheme.outline, fontWeight: FontWeight.w500)),
                  Text('$completedTasks من $totalTasks مهام', style: AppTheme.labelSm.copyWith(color: AppTheme.outline, fontWeight: FontWeight.w500)),
                ],
              ),
              const SizedBox(height: 8),
              ClipRRect(
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: motivationalColor.withValues(alpha: 0.2)),
            ),
            child: Row(
              children: [
                Container(
  ...
  Widget _buildStatsRow(TaskController controller, int totalTasks, int completedTasks) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          _buildStatCard('المهام المتبقية', '${pendingTasks}', Icons.pending_actions, AppTheme.statusPending),
          const SizedBox(width: 12),
          _buildStatCard('المهام المكتملة', '$completedTasks', Icons.check_circle, AppTheme.statusCompleted),
          const SizedBox(width: 12),
...
onStatusChanged: () async {
  ...
  if (oldStatus == TaskStatus.inProgress && updatedTask.status == TaskStatus.completed) {
    ...
    Get.snackbar('🎉 إنجاز!', 'تم إكمال مهمة: ${tasks[index].title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.statusCompleted,
      colorText: Colors.white,
      duration: const Duration(seconds: 3),
  ...
  // ✅ عند بدء المهمة (new → inProgress)
  else if (oldStatus == TaskStatus.new_ && updatedTask.status == TaskStatus.inProgress) {
    ...
    Get.snackbar('🚀 بدء العمل!', 'بدأت العمل على مهمة: ${tasks[index].title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.statusPending,
      colorText: Colors.white,
    );
  ...
  // ✅ عند إعادة فتح المهمة المكتملة (completed → أي حالة أخرى)
    ...
    Get.snackbar('🔄 إعادة فتح', 'تم إعادة فتح مهمة: ${tasks[index].title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.statusPending,
      colorText: Colors.white,
      duration: const Duration(seconds: 2),
  ...
  // ✅ عند إعادة فتح مهمة فائتة (missed → inProgress)
  else if (oldStatus == TaskStatus.missed && updatedTask.status == TaskStatus.inProgress) {
    await SoundService.playFocusStartSound();
    ...
    Get.snackbar('🔄 إعادة فتح', 'تم إعادة فتح المهمة الفائتة: ${tasks[index].title}',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppTheme.primary,
      colorText: Colors.white,
    );
